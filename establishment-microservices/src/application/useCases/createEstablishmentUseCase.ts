...
export default class CreateEstablishmentUseCase implements useCase {
  ...
  async execute(establishment: establishmentDto): Promise<boolean | { id: string; email: string; }> {
    ...
    const establishmentEntity = new EstablishmentEntity({...establishment, subsidiary: establishmentAddresses, password: await this.encoder.hash(establishment.password)});
